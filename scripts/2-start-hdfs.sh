...
#namenode
docker run -d --net dock_net --hostname namenode-master --mount type=bind,src="$(pwd)/data",target="/root/data" -p 9870:9870 -p 50030:50030 -p 8020:8020 --name namenode namenode:latest

#datanode 1
docker run -d --net dock_net --name datanode1 datanode:latest

#datanode 2
docker run -d --net dock_net --name datanode2 datanode:latest
